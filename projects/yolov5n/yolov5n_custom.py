...
class SPPFBlock(layers.Layer):
    ...
    def __init__(self, c_in, c_out, k=5):
        ...
        self.cv1 = ConvBlock(c_in, c_hidden, 1, 1, act="swish")
        self.cv2 = ConvBlock(c_hidden * 4, c_out, 1, 1, act="swish")
...
class Yolov5n:
    def __init__(self, input_shape, batch_size=1):
        ...
        x3 = Concat()([x3,x1])
        x3 = C3Block(c_in=256, c_out=128, n=1, shortcut=False, act="swish")(x3)
        x3 = ConvBlock(c_in=128, c_out=64, k=1, s=1, pad=1, act="swish")(x3)
        x4 = Upsample(scale_factor=2, mode="nearest")(x3)
        x4 = Concat()([x4,x])
        y1 = C3Block(c_in=128, c_out=64, n=1, shortcut=False, act="swish")(x4)
        x4 = ConvBlock(c_in=64, c_out=64, k=3, s=2, pad=1, act="swish")(y1)
        x4 = Concat()([x4,x3])
        y2 = C3Block(c_in=64, c_out=128, n=1, shortcut=False, act="swish")(x4)
        x4 = ConvBlock(c_in=128, c_out=128, k=3, s=2, pad=1, act="swish")(y2)
        x4 = Concat()([x4,x2])
        y3 = C3Block(c_in=256, c_out=256, n=1, shortcut=False, act="swish")(x4)
...
if __name__ == "__main__":
    ...
    yolov5n = create_yolov5n(image_size, model_path=r"E:\Models\yolov5\yolov5n_weights.npy")
    input = tf.ones((1, *image_size))
    y = yolov5n.model(input)
    yolov5n.save_weights()
    print("Done! Model weights are saved!")
