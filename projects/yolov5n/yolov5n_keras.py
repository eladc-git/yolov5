...
def ConvBlock(x, c_in, c_out, k=1, s=1, pad=None, act=None):
    # Standard convolution
    if s > 1:
        pad = autopad(k, pad)
        x = layers.ZeroPadding2D(padding=(pad, pad))(x)
    x = layers.Conv2D(c_out, k, s, 'SAME' if s == 1 else 'VALID', use_bias=True, activation=act)(x)
    return x
...
def C3Block(x, c_in, c_out, n=1, shortcut=True, act=None, e=0.5):
    ...
    y = layers.Concatenate(axis=3)([y, z])
...
def Concat(x):
    return layers.Concatenate(axis=3)(x)


def OutputLayer(x):
    assert len(x) == 3, "Error: input length must be 3"
    x1 = ConvBlock(x[0], 256, 255, 1, 1, act="sigmoid")
    x1 = layers.Reshape((-1, 3, 85))(x1)
    #x1 = layers.Permute([2, 1, 3])(x1)
    ...
    #x3 = layers.Permute([2, 1, 3])(x3)

    return [x1,x2,x3]


class Yolov5n:
    def __init__(self, input_shape, batch_size=None):
        self.input_shape = input_shape
        inputs = layers.Input(shape=input_shape, batch_size=batch_size)
        x = ConvBlock(inputs, c_in=3, c_out=16, k=6, s=2, pad=2, act="relu")
        x = ConvBlock(x, c_in=16, c_out=32, k=3, s=2, pad=1, act="relu")
...
def create_yolov5n(image_size, batch_size=None, weights_pickle_path=None):
