...
from models.common import C3, SPP, SPPF, Bottleneck, BottleneckCSP, Conv, Concat, DWConv, Focus, autopad
...
class TFConv(layers.Layer):
    ...
    def __init__(self, c1, c2, k=6, s=1, p=None, g=1, act=True, w=None):
        ...
        self.conv = self.conv if s == 1 else keras.Sequential([TFPad(autopad(k, p)), self.conv])
    ...
    def get_config(self):
        config = super().get_config()
        config.update({
            "Cin": self.c1, "Cout": self.c2, "kernel": self.k, "stride": self.s,
            "BN": False if self.bn==tf.identity else True,
            "Activation": False if self.act==tf.identity else self.act_fun,
        })
        return config
...
class TFBottleneck(layers.Layer):
    # Standard bottleneck
...
class TFC3(layers.Layer):
    ...
    def get_config(self):
        config = super().get_config()
        config.update({
            "Conv1": self.cv1, "Conv2": self.cv2, "Conv3": self.cv2, "Bottleneck": self.m,
        })
        return config

class TFSPPF(layers.Layer):
    ...
    def get_config(self):
        config = super().get_config()
        config.update({
            "Conv1": self.cv1, "Conv2": self.cv2, "MaxPooling": self.m,
        })
        return config

class TFDetect(layers.Layer):
...
def parse_model(d, ch, model, imgsz):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        keras_m = eval('TF' + m_str.replace('nn.', ''))
        m_ = keras.Sequential([keras_m(*args, w=model.model[i][j]) for j in range(n)]) if n > 1 \
            else keras_m(*args, w=model.model[i])  # module
...
class KerasModel:
    ...
    def predict(self, inputs):
        ...
        return x  #x[0]  # output only first tensor [1,6300,85] = [xywh, conf, class0, class1, ...]
...
def run(weights=ROOT / 'yolov5s.pt',  # weights path
    ...
    tf_model = KerasModel(cfg=model.yaml, model=model, nc=model.nc, imgsz=imgsz)
